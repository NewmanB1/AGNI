...
| Change sensor behaviour or add a sensor | `packages/agni-runtime/sensors/sensor-bridge.js` — publishes into `AGNI_SHARED`. DeviceMotion/orientation are throttled to 100 ms to avoid event-loop exhaustion on Chrome 51. See **`sensor-toolkit-improvement-plan.md`** for planned enhancements. |
