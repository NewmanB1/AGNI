...
| Change sensor behaviour or add a sensor | `packages/agni-runtime/sensors/sensor-bridge.js` — publishes into `AGNI_SHARED`. Ensure any new runtime module that needs sensors subscribes via `AGNI_SHARED` and loads after `sensor-bridge.js`. See **`sensor-toolkit-improvement-plan.md`** for planned enhancements and verification requirements. |
...
| Change integrity verification | `packages/agni-runtime/integrity/integrity.js` (`AGNI_INTEGRITY`) and `packages/agni-utils/crypto.js` (signContent). Binding hash contract must stay identical on both sides. |
