...
2. Compiler builds `gravity.html` + `lesson-ir.json` sidecar via `@ols/compiler` (packages/ols-compiler).
...
- **Runtime cycle handling (graceful degradation):** When a cycle is detected (e.g. A→B→C→A), theta prunes the cycle nodes from the graph and excludes lessons that provide those skills. The hub continues running; affected lessons are ineligible until the curriculum is fixed. Set `AGNI_STRICT_SKILL_GRAPH=1` to restore strict mode (throw on cycle). See `docs/archive/ARCHITECTURAL-VULNERABILITIES-REMEDIATION-PLAN.md`.
- **Compile-time DAG validation:** `scripts/check-skill-dag.js` validates the skill graph for cycles. Run `npm run verify:skill-dag`; included in `verify:all`.
