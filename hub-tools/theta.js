...
if (lmsEngine.isAvailable()) {
  ...
  log.warn('LMS engine not available — degraded mode: theta scheduling active, bandit selection disabled');
}
...
function expandScheduledSkills(initialSkills, studentSkills, skillGraph) {
  ...
  if (depth >= MAX_DEPTH) log.warn('BFS depth limit reached — possible cycle in skill graph');
  return expanded;
}
