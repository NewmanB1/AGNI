...
(function (global) {
  ...
  function _getPseudoId() {
    ...
    return _dbGet(STORE_META, 'pseudoId').then(function (stored) {
      ...
      // ES5-safe pseudoId generation — no Array.from or padStart [R10 P4.2]
      var bytes = global.crypto
        ? global.crypto.getRandomValues(new Uint8Array(4))
        : new Uint8Array([Math.random()*256, Math.random()*256, Math.random()*256, Math.random()*256]);
      var hex = '';
      for (var bi = 0; bi < bytes.length; bi++) {
        var h = (bytes[bi] & 0xff).toString(16);
        hex += (h.length < 2 ? '0' : '') + h;
      }
      var id = 'px-' + hex;
      _pseudoId = id;
