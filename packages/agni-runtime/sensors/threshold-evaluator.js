...
(function (global) {
  ...
  function buildEvaluator(node) {
    ...
    if (node.type === 'and') {
      ...
      // Steady evaluators need the other side's result to know whether
      // to advance their timer. Handle steady on either side. [R10 P4.3]
      var leftIsSteady  = (node.left.type === 'steady');
      var rightIsSteady = (node.right.type === 'steady');

      return function (sensorValues) {
        if (leftIsSteady && !rightIsSteady) {
          var rightResult = rightFn(sensorValues);
          var leftResult  = leftFn(sensorValues, rightResult);
          return leftResult && rightResult;
        }
        if (rightIsSteady && !leftIsSteady) {
          var leftResult2  = leftFn(sensorValues);
          var rightResult2 = rightFn(sensorValues, leftResult2);
          return leftResult2 && rightResult2;
        }
        // Neither or both are steady — evaluate normally
        var lr = leftFn(sensorValues);
        if (!lr) { rightFn(sensorValues, false); return false; }
        var rr = rightFn(sensorValues);
        return lr && rr;
      };
    }
