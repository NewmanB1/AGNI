...
(function (global) {
  ...
  // ── Gauge ──────────────────────────────────────────────────────────────────
  // Dial gauge / meter for displaying a single sensor value in real time.
  // Inspired by Phyphox instrument panels: arc needle, numeric readout,
  ...
  SVG.gauge = function (stage, opts) {
    ...
    var maxVal = opts.max != null ? opts.max : 10;
    var startAngle = -225, endAngle = 45;
    var sweep = endAngle - startAngle;
    var title = opts.title || '';
    var unit = opts.unit || '';
    var zones = opts.zones || [];
    var sensorId = opts.sensor || null;
    var currentValue = opts.value != null ? opts.value : minVal;

    var g = stage.layer('gauge');
    ...
    function arcPath(cx, cy, r, startDeg, endDeg) {
      var s = polarToCart(cx, cy, r, startDeg);
      var e = polarToCart(cx, cy, r, endDeg);
    ...
    var track = el('path', {
      ...
      fill: 'none', stroke: '#dddddd', 'stroke-width': '8', 'stroke-linecap': 'round'
    });
    g.appendChild(track);
    ...
    var renderZones = zones.length > 0 ? zones : DEFAULT_ZONES;
    renderZones.forEach(function (z) {
      var fromAngle = startAngle + (z.from || 0) * sweep;
      var toAngle = startAngle + Math.min(z.to || 1, 1) * sweep;
      if (toAngle <= fromAngle) return;
      var zoneEl = el('path', {
        d: arcPath(CX, CY, R, fromAngle, toAngle),
        fill: 'none', stroke: z.color || '#cccccc', 'stroke-width': '8', 'stroke-linecap': 'round',
        opacity: '0.35'
      });
    ...
    // Tick marks
    var tickCount = opts.ticks != null ? opts.ticks : 5;
    for (var i = 0; i <= tickCount; i++) {
      var pct = i / tickCount;
      var tAngle = startAngle + pct * sweep;
      var inner = polarToCart(CX, CY, R - 12, tAngle);
      var outer = polarToCart(CX, CY, R + 2, tAngle);
      g.appendChild(el('line', {
        x1: inner.x.toFixed(1), y1: inner.y.toFixed(1),
        x2: outer.x.toFixed(1), y2: outer.y.toFixed(1),
        stroke: '#888888', 'stroke-width': '1.5'
      }));
      var lbl = polarToCart(CX, CY, R - 22, tAngle);
      var tickVal = minVal + pct * (maxVal - minVal);
      g.appendChild(txt(tickVal % 1 === 0 ? String(tickVal) : tickVal.toFixed(1), {
        x: lbl.x.toFixed(1), y: lbl.y.toFixed(1),
        'font-size': '9', fill: '#666666'
      }));
    }
    ...
    // Center hub
    g.appendChild(el('circle', { cx: CX, cy: CY, r: '5', fill: '#333333' }));

    // Numeric readout
    var readout = txt('--', {
      x: CX, y: CY + R * 0.5,
      'font-size': '18', 'font-weight': 'bold', fill: '#333333'
    });
    g.appendChild(readout);

    // Unit label
    if (unit) {
      g.appendChild(txt(unit, {
        x: CX, y: CY + R * 0.5 + 16,
        'font-size': '10', fill: '#888888'
      }));
    }
    ...
    if (title) {
      g.appendChild(txt(title, {
        x: CX, y: 14,
        'font-size': '13', 'font-weight': 'bold', fill: '#444444'
      }));
    }

    function updateNeedle(val) {
      currentValue = val;
      var angle = angleForValue(val);
      var tip = polarToCart(CX, CY, R - 14, angle);
      needle.setAttribute('x2', tip.x.toFixed(1));
      needle.setAttribute('y2', tip.y.toFixed(1));
      readout.textContent = typeof val === 'number' ? val.toFixed(1) : '--';
    }

    updateNeedle(currentValue);

    if (sensorId) {
      var sMin = opts.sensorMin != null ? opts.sensorMin : minVal;
      var sMax = opts.sensorMax != null ? opts.sensorMax : maxVal;
      stage.bindSensor(sensorId, function (reading) {
        var mapped = minVal + ((reading.value - sMin) / (sMax - sMin)) * (maxVal - minVal);
        mapped = Math.max(minVal, Math.min(maxVal, mapped));
        updateNeedle(mapped);
      });
    } else if (opts.animate) {
      var dir = 1;
      stage.onTick(function (t) {
        currentValue += dir * (opts.speed || 1) * 0.016;
        if (currentValue >= maxVal) { currentValue = maxVal; dir = -1; }
        if (currentValue <= minVal) { currentValue = minVal; dir = 1; }
        updateNeedle(currentValue);
      });
    }

    return { update: updateNeedle, getValue: function () { return currentValue; } };
  };
  ...
  if (global.DEV_MODE) {
    console.log('[SVG-DYNAMIC] loaded –', ['numberLineDynamic','clockFaceDynamic','timeGraph','arrowMap','gauge','compose'].join(', '));
  }
