...
(function (global) {
  ...
  function _loadKatex() {
    ...
    fetchKatex.then(function (jsText) {
      // Load KaTeX via <script> element instead of eval/new Function
      // to comply with CSP script-src restrictions.
      try {
        var script = document.createElement('script');
        script.textContent = jsText;
        if (global.AGNI_CSP_NONCE) script.setAttribute('nonce', global.AGNI_CSP_NONCE);
        document.head.appendChild(script);
      } catch (e) { log.warn('KaTeX load error:', e.message); return; }
      _katexLoaded  = true;
      _katexLoading = false;
      _injectKatexCSS();
