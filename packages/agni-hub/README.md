...
When adding a new API endpoint, create a route file in `packages/agni-hub/routes/`. Routes are auto-loaded by `pathfinder.js` (the main server entry point); no manual registration needed. Keep business logic in the appropriate package (engine, governance, compiler) and use routes only for HTTP wiring.

## Types

`index.d.ts` provides minimal type declarations for @agni/hub. For full typing, prefer `@agni/services` subpaths.
